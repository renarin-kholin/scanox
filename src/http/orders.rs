...
use crate::http::razorpay::{decrypt_order, QRPayloadData};
...
async fn auth( ctx: Extension<ApiContext>,mut req: Request,  next: Next) -> std::result::Result<Response, StatusCode> {
...
async fn collect_order(
    ...
    let order_id = Uuid::from_str(&order_id).map_err(|_| anyhow!("Invalid order_id"))?;
...
#[axum::debug_handler]
async fn verify_qrcode(ctx: Extension<ApiContext>,  axum::extract::Json(payload): axum::extract::Json<VerifyQRCodeRequestBody>) -> Result<Json<VerifyQRCodeResponse>>{
    let qr_payload: QRPayloadData = decrypt_order(&ctx.cipher, payload.qrcode_data).await?;
    let order = sqlx::query!(r#"select is_received, is_paid, order_id, copies, is_color, is_both_side from "order" where razorpay_order_id = $1"#, qr_payload.razorpay_order_id).fetch_one(&ctx.db).await?;
    let mut verify_response = VerifyQRCodeResponse {
        info: None,
        verified: true,
    };
    let is_expired = SystemTime::now().duration_since(qr_payload.created_at).map_err(|_| anyhow!("could not compare times"))? > Duration::DAY;
    ...
    if verified {
        verify_response.info = Some(OrderDetails {
            ...
            is_color: order.is_color.ok_or( anyhow!("Could not get order details"))?,
            is_both_side: order.is_both_side.ok_or( anyhow!("Could not get order details"))?
        })
    }
