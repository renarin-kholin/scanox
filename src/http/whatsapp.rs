use crate::http::types::{
    EventNotificationFb, EventNotificationsContactProfileFb, EventNotificationsDocumentFb, ItemType,
};
    ...
    if document.filename.ends_with(".pdf") {
        ...
        send_message(
            ...
            SendMessageType::Text(
                &TextMessage::from(
                    "Starting a new print order, Please enter the number of copies you want.",
                )
                .as_string()?,
            ),
...
async fn handle_text_message(
    ...
    if let Some(order_id) = uo.get(&from) {
        let order = sqlx::query!(
            r#" select progress as "progress!: OrderProgress" from "order" where order_id=$1"#,
            order_id
        )
        ...
        match order.progress {
            OrderProgress::Started => {
                ...
                if copies.is_err() {
                    send_message(
                        ...
                        SendMessageType::Text(
                            &TextMessage::from(
                                "That is not a valid number. Proceeding with 1 copies.",
                            )
                            .as_string()?,
                        ),
                        &ctx.config.whatsapp_token,
                ...
                if let Ok(copies) = copies {
                    sqlx::query_scalar!(
                        r#"update "order" set copies = $1, progress='COPIES' where order_id = $2"#,
                        copies as i32,
                        order_id
                    )
                    .fetch_optional(&ctx.db)
                    ...
                    send_message(
                        from,
                        SendMessageType::Text(
                            &TextMessage::from("Do you want both side print? Answer Yes or No")
                                .as_string()?,
                        ),
                        &ctx.config.whatsapp_token,
                    )
            ...
            OrderProgress::Copies => {
                ...
                send_message(
                    from.clone(),
                    SendMessageType::Text(
                        &TextMessage::from("Generating payment link, please wait.").as_string()?,
                    ),
                    &ctx.config.whatsapp_token,
                )
                .await?;
                if let (Some(is_color), Some(is_both_side), Some(copies)) =
                    (order.is_color, order.is_both_side, order.copies)
                {
                    let item_type = ItemType::from(is_color, is_both_side);
                    let item = ctx.razorpay_items.get(&item_type).ok_or(Error::NotFound)?;
                    let razorpay_res = generate_invoice(
                        InvoiceOrderDetails {
                    ...
                    .await?;
                    sqlx::query!(
                        r#"update "order" set razorpay_order_id = $1 where order_id = $2"#,
                        razorpay_res.order_id,
                        order_id
                    )
                    .fetch_optional(&ctx.db)
                    .await?;
                    send_message(
                        from,
                        SendMessageType::Text(
                            &TextMessage::from(razorpay_res.short_url.as_str()).as_string()?,
                        ),
                        &ctx.config.whatsapp_token,
                    )
                    .await?;
...
async fn post_whatsapp(
    ...
    let event_notification = <EventNotificationFb as Deserialize>::deserialize(payload)
        .map_err(|_| Error::unprocessable_entity([("deserialize", "could not deserialize")]))?;
    if !event_notification.entry.is_empty() && !event_notification.entry[0].changes.is_empty() {
        ...
        if let Some(en_value) = en_value {
            if !en_value.messages.is_empty() {
                ...
                let mut user_name = en_value.contacts[0]
                    .clone()
                    .profile
                ...
                if let Some(document) = document {
                    ...
                    return handle_text_message(ctx, state, text.body, user_name, from_number)
                        .await;
                }
            }
        }
